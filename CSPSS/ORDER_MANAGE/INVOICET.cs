...
{
    public partial class INVOICET : Form
    {
        ...
        public INVOICET(INVOICE Frm)
        {
            InitializeComponent();
            F1 = Frm;
        }
        private void INVOICET_Load(object sender, EventArgs e)
        {
        ...
        private void right()
        {
            DataTable dtx = cedit_right.RETURN_RIGHT_LIST("开票维护", LOGIN.USID);
        ...
        private void bind()
        {
          this.Icon = Resource1.xz_200X200;
            ...
            DataTable dtx = basec.getdts(cinvoice.sql +" where A.IVID='" + IDO + "' ORDER BY  B.IVKEY ASC ");
            if (dtx.Rows.Count > 0)
            {
                comboBox1.Text = dtx.Rows[0]["订单编号"].ToString();
                dt = cinvoice.GET_CALCULATE(dtx);
            ...
            sqb.Append(cinvoice.sqlo);
            sqb.Append(" WHERE C.ORDER_ID='" + comboBox1.Text + "'");
            sqb.Append(" GROUP BY C.ORDER_ID,E.CName,C.WAREID,C.HAVE_TAX_UNIT_PRICE,");
            sqb.Append(" C.PRODUCTION_COUNT,B.TAX_RATE  ORDER BY C.ORDER_ID ASC");
            dtx = bc.getdt(sqb.ToString());
            if (dtx.Rows.Count > 0)
            {
               
                textBox6.Text = dtx.Rows[0]["实开票金额"].ToString();
                textBox7.Text = dtx.Rows[0]["待开金额"].ToString();
            }
        ...
        private void dgvStateControl()
        {
            ...
            dataGridView1.Columns["开票日期"].Width = 100;
            dataGridView1.Columns["领票日期"].Width = 100;
            ...
            dataGridView1.Columns["税金"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            dataGridView1.Columns["含税金额"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            dataGridView1.Columns["待开金额"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
            ...
            dataGridView1.Columns["开票日期"].DefaultCellStyle.BackColor = CCOLOR.CUSTOMER_YELLOW;
            dataGridView1.Columns["未税金额"].DefaultCellStyle.BackColor = CCOLOR.CUSTOMER_YELLOW;
            dataGridView1.Columns["税率"].DefaultCellStyle.BackColor = CCOLOR.CUSTOMER_YELLOW;
            dataGridView1.Columns["发票号码"].DefaultCellStyle.BackColor = CCOLOR.CUSTOMER_YELLOW;
            dataGridView1.Columns["项次"].ReadOnly = true;
            dataGridView1.Columns["税金"].ReadOnly = true;
            dataGridView1.Columns["含税金额"].ReadOnly = true;
            dataGridView1.Columns["待开金额"].ReadOnly = true;
        ...
        {
            string varDate = DateTime.Now.ToString("yyy/MM/dd").Replace("-", "/");
            DataTable dtt2 = cinvoice.GetTableInfo();
            for (i = 1; i <= 6; i++)
            {
        ...
        private bool juage()
        {
            ...
            else if (ADD_OR_UPDATE!="UPDATE" && bc.exists(cinvoice.sql + " WHERE C.ORDER_ID='"+comboBox1.Text +"'"))
            {
                hint.Text = string.Format("订单编号 {0} 已经有开票记录，请查询该订单编号后双击进入修改", comboBox1.Text);
                b = true;
            }
            ...
            {
                DataTable  dtx = bc.GET_NOEXISTS_EMPTY_ROW_DT(dt, "", "未税金额 IS NOT NULL");
                if (dtx.Rows.Count > 0)
                {
                    for (i = 0; i < dtx.Rows.Count; i++)
                    {
                        ...
                        if (!DateTime.TryParse(dtx.Rows[i]["开票日期"].ToString(), out temp))
                        {
                            hint.Text = string.Format("第 {0} 行开票日期格式不正确 需为格式yyyy/MM/dd", i + 1);
                            b = true;
                            break;
                        }
                       else if (string.IsNullOrEmpty(dtx.Rows[i]["未税金额"].ToString()))
                        {
                            hint.Text = string.Format("第 {0} 行未税金额不能为空", i + 1);
                            b = true;
                            break;
                        }
                        else if (bc.yesno(dtx.Rows[i]["未税金额"].ToString())==0)
                        {
                            hint.Text = string.Format("第 {0} 行未税金额只能输入数字", i + 1);
                            b = true;
                            break;
                        }
                        ...
                        else if (string.IsNullOrEmpty(dtx.Rows[i]["发票号码"].ToString()))
                        {
                            hint.Text = string.Format("第 {0} 行发票号码不能为空", i + 1);
                            b = true;
                            break;
                        }
                        else if (dtx.Rows[i]["领票日期"].ToString()!="" && !DateTime.TryParse(dtx.Rows[i]["领票日期"].ToString(), out temp))
                        {
                            hint.Text = string.Format("第 {0} 行领票日期格式不正确 需为格式yyyy/MM/dd", i + 1);
                            b = true;
                            break;
                        }
                        else if (!string.IsNullOrEmpty(dtx.Rows[i]["待开金额"].ToString()))
                        {
                            WAIT_NOTICE_AMOUNT = decimal.Parse(dtx.Rows[i]["待开金额"].ToString());
                            if (WAIT_NOTICE_AMOUNT < 0)
                            {
                                hint.Text = string.Format("第 {0} 行待开金额不能出现负数", i + 1);
                                b = true;
                                break;
                            }
                        }
        ...
        private void save()
        {
            try
            {
                ...
                DataTable dtt = bc.getdt(cinvoice.sql + " WHERE A.IVID='" + IDO + "'");
                if (dtt.Rows.Count > 0)
                {
                ...
                else
                {
                    ...
                    {
                        cinvoice.IVID = IDO;
                        cinvoice.PNID = bc.getOnlyString("SELECT PNID FROM PN_PRODUCTION_INSTRUCTIONS WHERE ORDER_ID='" + comboBox1.Text + "'");
                        cinvoice.EMID = LOGIN.EMID;
                        cinvoice.IF_IMPORT = false;
                        cinvoice.save("INVOICE_MST", "INVOICE_DET", "IVID", IDO, dtx);
                    ...
                    {
                        hint.Text = "至少有一项才能保存且该项未税金额栏位不能为空！";
        ...
        private void btnDel_Click(object sender, EventArgs e)
        {
            try
            {
               if (MessageBox.Show("确定要删除该条凭证吗？", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                {
                    basec.getcoms("DELETE INVOICE_MST WHERE IVID='" + IDO+ "'");
                    basec.getcoms("DELETE INVOICE_DET WHERE IVID='" + IDO + "'");
                    bind();
                    ClearText();
                    textBox2.Text = "";
                    F1.bind();
        ...
        private void dataGridView1_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            try
            {
                ...
                if (dataGridView1.Columns[columnsindex].Name == "未税金额" && bc.yesno(e.FormattedValue.ToString()) == 0)
                {
                    e.Cancel = true;
        ...
        private void dataGridView1_RowValidating(object sender, DataGridViewCellCancelEventArgs e)
        {
            try
            {
                ...
                string v1 = dataGridView1["未税金额", rowsindex].FormattedValue.ToString();
                string v2 = dataGridView1["税率", rowsindex].FormattedValue .ToString();
                string v3=dataGridView1["发票号码", rowsindex].FormattedValue .ToString();
                string v4 = dataGridView1["开票日期", rowsindex].FormattedValue.ToString();
                if (v1 == "")
                {
                }
                else  if (bc.exists (cinvoice .sql+" WHERE C.ORDER_ID='"+comboBox1 .Text +"'") && textBox5.Text == "")
                {
                    hint.Text = "订单金额不能为空";
                }
                ...
                {
                    string v5 = dataGridView1["开票日期", rowsindex-1].FormattedValue.ToString();
                    if (v5 =="")
                    {
                        //e.Cancel = true;
        ...
        private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                string v1 = dataGridView1["未税金额", rowsindex].Value.ToString();
                string v2 = dataGridView1["税率", rowsindex].Value.ToString();
                DateTime temp = DateTime.MinValue;
                if (dataGridView1["未税金额", rowsindex].FormattedValue.ToString() != "")
                {
                    d1 = decimal.Parse(dt.Rows[rowsindex]["未税金额"].ToString());
                }
                if (dataGridView1["税率", rowsindex].FormattedValue.ToString() != "")
                {
                    d2 = decimal.Parse(dt.Rows[rowsindex]["税率"].ToString());
                }
                if (rowsindex == 0)
                {
                    if (d1 * d2 / 100 > 0)
                    {
                        dataGridView1["税金", rowsindex].Value = (d1 * d2 / 100).ToString("0.00");
                        dataGridView1["含税金额", rowsindex].Value = (d1 * (1 + d2 / 100)).ToString("0.00");
                        if (ORDER_AMOUNT > 0)
                        {
                            dataGridView1["待开金额", rowsindex].Value = (ORDER_AMOUNT - (d1 * (1 + d2 / 100))).ToString("0.00");
                ...
                else
                {
                    if (d1 * d2 / 100 > 0)
                    {
                        if (dataGridView1["待开金额", rowsindex - 1].FormattedValue.ToString() != "")
                        {
                            d3 = decimal.Parse(dt.Rows[rowsindex - 1]["待开金额"].ToString());
                        }
                        dataGridView1["税金", rowsindex].Value = (d1 * d2 / 100).ToString("0.00");
                        dataGridView1["含税金额", rowsindex].Value = (d1 * (1 + d2 / 100)).ToString("0.00");
                        ...
                        if (ORDER_AMOUNT > 0)
                        {
                            dataGridView1["待开金额", rowsindex].Value = (d3 - (d1 * (1 + d2 / 100))).ToString("0.00");
