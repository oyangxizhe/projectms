...
ORDER_TYPE A ";
        ...
        private void save()
        {
            ...
            if (!bc.exists("SELECT OTID FROM ORDER_TYPE WHERE OTID='" + textBox1 .Text + "'"))
            {

                if (bc.exists("SELECT * FROM ORDER_TYPE WHERE ORDER_TYPE='"+textBox2 .Text +"'"))
                {
                    hint.Text = "此订单类型已经存在";
                ...
                {
                    basec.getcoms(@"INSERT INTO ORDER_TYPE(OTID,ORDER_TYPE,MAKERID,DATE,YEAR,
            ...
            else
            {
                ...
                {
                    basec.getcoms(@"UPDATE ORDER_TYPE SET ORDER_TYPE='" + textBox2.Text + "',MAKERID='" + varMakerID +
                          "',DATE='" + varDate + "' WHERE OTID='" + textBox1.Text + "'");
                    IFExecution_SUCCESS = true;
                    bind();
                }
            }
        ...
        {
            ClearText();
            textBox1.Text = cORDER_TYPE.GETID();
            textBox2.Focus();
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dt = bc.getdt(sql+" WHERE A.OTID LIKE '%"+textBox4.Text +"%' AND A.ORDER_TYPE LIKE '%"+textBox5 .Text +"%'");
                if (dt.Rows.Count > 0)
                {
                    dataGridView1.DataSource = dt;
                    dgvStateControl();
        ...
        private void btnDel_Click(object sender, EventArgs e)
        {
            ...
            {
                IFExecution_SUCCESS = false;
                string strSql = "DELETE FROM ORDER_TYPE WHERE OTID='" + id + "'";
        ...
        private void ORDER_TYPE_Load(object sender, EventArgs e)
        {
