...
{
    public partial class COLOR_PARAMETERS : Form
    {
...
SELECT 
A.COLOR_PARAMETERS AS 专名,
A.COLOR_COUNT AS 色数,
A.CTP_EDITION AS CTP版,
...
COLOR_PARAMETERS A ";
        ...
        private void dgvStateControl()
        {
            ...
            for (i = 0; i < numCols1; i++)
            {
                ...
                dataGridView1.Columns["专名"].Width = 120;
        ...
        private void save()
        {
            ...
            string GET_COLOR_PARAMETERS = bc.getOnlyString("SELECT COLOR_PARAMETERS FROM COLOR_PARAMETERS WHERE CPID='" + textBox1.Text + "'");
            if (!bc.exists("SELECT CPID FROM COLOR_PARAMETERS WHERE CPID='" +textBox1 .Text  + "'"))
            {

                if (bc.exists("SELECT * FROM COLOR_PARAMETERS WHERE COLOR_PARAMETERS='"+textBox2 .Text +"'"))
                {
                    hint.Text = "此专色已经存在";
                ...
                {
                    basec.getcoms(@"INSERT INTO COLOR_PARAMETERS(CPID,COLOR_PARAMETERS,COLOR_COUNT,CTP_EDITION,MAKERID,DATE,YEAR,
                                   MONTH) VALUES ('" +IDO+ "','" + textBox2.Text +
                     "','"+textBox3 .Text +"','"+textBox6 .Text +"','" + varMakerID + "','" + varDate +
                     "','" + year + "','" + month + "')");
            ...
            else if (GET_COLOR_PARAMETERS != textBox2.Text)
            {
                if (bc.exists("SELECT * FROM COLOR_PARAMETERS WHERE COLOR_PARAMETERS='" + textBox2.Text + "'"))
                {
                    hint.Text = "此专色已经存在";
                ...
                {
                    basec.getcoms(@"UPDATE COLOR_PARAMETERS SET COLOR_PARAMETERS='" + textBox2.Text + "',COLOR_COUNT='" + textBox3.Text +
                        "' ,CTP_EDITION='" + textBox6.Text + "',MAKERID='" + varMakerID +
                          "',DATE='" + varDate + "' WHERE CPID='" + textBox1.Text + "'");
                    IFExecution_SUCCESS = true;
                    bind();
                }
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dt = bc.getdt(sql+" WHERE  A.COLOR_PARAMETERS LIKE '%"+textBox5 .Text +"%'");
                if (dt.Rows.Count > 0)
                {
                    dataGridView1.DataSource = dt;
                    dgvStateControl();
        ...
        private void btnDel_Click(object sender, EventArgs e)
        {
            ...
            {
                IFExecution_SUCCESS = false;
                string strSql = "DELETE FROM COLOR_PARAMETERS WHERE COLOR_PARAMETERS='" + id + "'";
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (v1 != "")
            {
                textBox1.Text = bc.getOnlyString(string.Format("SELECT CPID FROM COLOR_PARAMETERS WHERE COLOR_PARAMETERS='{0}'",v1));
                ...
                textBox6.Text = Convert.ToString(dataGridView1[2, dataGridView1.CurrentCell.RowIndex].Value).Trim();
        ...
        private void dataGridView1_DoubleClick(object sender, EventArgs e)
        {
            ...
            OFFER_MANAGE.PRINTING_OFFERT.POSITIVE_COLOR = dt.Rows[index]["专名"].ToString();
