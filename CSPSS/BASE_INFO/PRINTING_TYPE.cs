...
{
    public partial class PRINTING_TYPE : Form
    {
        ...
        private void PRINTING_TYPE_Load(object sender, EventArgs e)
        {
        ...
        private void bind()
        {
            ...
            stb.Append(cPRINTING_TYPE.sql);
            stb.Append(" WHERE B.SIZE LIKE '%" + textBox2.Text + "%'");
            stb.Append(" AND B.MACHINE_TYPE LIKE '%" + textBox3.Text + "%'");
 ...
 (SELECT USER_GROUP FROM USERINFO WHERE USID='" + LOGIN.USID + "'))" + sqlo);
            ...
            //dt = cPRINTING_TYPE.RETURN_DT(dt);
            if (dt.Rows.Count > 0)
            {
        ...
        {
            //IDO = cPRINTING_TYPE.GETID();
            PRINTING_TYPET FRM = new PRINTING_TYPET(this);
            FRM.IDO = cPRINTING_TYPE.GETID();
        ...
        private void dataGridView1_DoubleClick(object sender, EventArgs e)
        {
            ...
            string s1 = this.dataGridView1.Rows[r].Cells[0].Value.ToString().Trim();
            string s2 = this.dataGridView1.Rows[r].Cells[1].Value.ToString().Trim();
            string s3 = this.dataGridView1.Rows[r].Cells[2].Value.ToString().Trim();
            string s4 = this.dataGridView1.Rows[r].Cells[3].Value.ToString().Trim();
           
            if (GET_DATA_OR_UPDATE)
            {
               
             
                    //OFFER_MANAGE.PRINTING_OFFERT.PRINTING_TYPE = s2;
            ...
            {
                PRINTING_TYPET FRM = new PRINTING_TYPET(this);
                FRM.IDO = bc.getOnlyString(string.Format ("SELECT * FROM PRINTING_TYPE_MST B" +@" 
WHERE B.MACHINE_TYPE='{0}' AND B.CUSTOMER_TYPE='{1}'",dt.Rows [r]["机型"].ToString (),dt.Rows [r]["客户类别"].ToString ()));
        ...
        private void btnToExcel_Click(object sender, EventArgs e)
        {
            if (dt.Rows.Count > 0)
            {
                ...
                bc.dgvtoExcel(dataGridView1, "印刷类信息");
