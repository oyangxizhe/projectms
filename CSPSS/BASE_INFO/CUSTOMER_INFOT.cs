...
namespace CSPSS.BASE_INFO
{
    public partial class CUSTOMER_INFOT : Form
    {
        ...
        private void bind()
        {
            ...
            if (dtx.Rows.Count > 0)
            {
                ...
                foreach (DataRow dr1 in dtx.Rows)
                {
                    ...
                    dr["用户工号"] = dr1["用户工号"].ToString();
                    dr["用户姓名"] = dr1["用户姓名"].ToString();
        ...
        private void save()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                DataTable dtx = bc.GET_NOEXISTS_EMPTY_ROW_DT(dt, "", "用户工号 IS NOT NULL");
                if (dtx.Rows.Count > 0)
                {
            ...
            foreach (DataRow dr in dtx.Rows)
            {
               ...
               /* else if (v5!="" && bc.exists("SELECT * FROM CUSTOMERINFO_DET WHERE QQ='" + v5 + "' AND CUID!='"+ textBox1 .Text +"'"))
                ...
                else if (v7!="" && v6 == "")
                {
                    ...
                    hint.Text = "项次" + dr["项次"].ToString() + " 当客户类别不为空时品牌不能为空";
                    b = true;
                }
               else if (v6!="" && v7 == "")
                {

                    hint.Text = "项次" + dr["项次"].ToString() + " 品牌不为空时客户类别不能为空";
                    b = true;
                }
        ...
        private void btnDel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (bc.exists("SELECT * FROM PROJECT_INFO WHERE CUID='"+textBox1 .Text +"'"))
                    {
                        hint.Text = "此客户信息在项目信息作业中存在不允许删除";
                    }
                    else if (bc.exists("SELECT * FROM OTHER_COST WHERE CUID='" + textBox1.Text + "'"))
                    {
                        hint.Text = "此客户信息在其它费用作业中存在不允许删除";
                    }
                    else
                    {
        ...
        private void dgvStateControl()
        {
           ...
           /*dataGridView1.Columns["项次"].Width = 40;
            ...
            dataGridView1.Columns["用户工号"].DefaultCellStyle.BackColor = Color.Yellow;
            dataGridView1.Columns["品牌"].DefaultCellStyle.BackColor = Color.Yellow;
            dataGridView1.Columns["客户类别"].DefaultCellStyle.BackColor = Color.Yellow;
        ...
        private void dataGridView1_Click(object sender, EventArgs e)
        {
            ...
            {
                TEMP FRM = new TEMP();
                FRM.PARAMETERS_SELECT = 0;
                FRM.ShowDialog();
                if (IF_DOUBLE_CLICK)
                {
                    dt.Rows[rows]["客户类别"] = RETURN_DATA;
                }
                dataGridView1.CurrentCell = dataGridView1["部门", rows];
            }
            else if (dataGridView1.Columns[columns].DataPropertyName.ToString() == "用户工号")
            {
                EMPLOYEE_INFO FRM = new EMPLOYEE_INFO();
                FRM.CUSTOMERINFO_USE();
                FRM.ShowDialog();
                if (IF_DOUBLE_CLICK)
                {
                    dt.Rows[rows]["用户工号"] = EMPLOYEE_ID;
                    dt.Rows[rows]["用户姓名"] = ENAME;
                }
                dataGridView1.CurrentCell = dataGridView1["联系电话", rows];
            }
        }
