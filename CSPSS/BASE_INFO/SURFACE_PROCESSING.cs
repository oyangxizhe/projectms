...
{
    public partial class SURFACE_PROCESSING : Form
    {
...
A.DATE AS 制单日期
FROM
SURFACE_PROCESSING A ";
   
        protected int M_int_judge, i;
        protected int select;
        public SURFACE_PROCESSING()
        {
            InitializeComponent();
        }
        ...
        private void dgvStateControl()
        {
            ...
            for (i = 0; i < numCols1; i++)
            {
                ...
                dataGridView1.Columns["表面加工"].Width = 120;
        ...
        private void save()
        {
            ...
            if (!bc.exists("SELECT SFID FROM SURFACE_PROCESSING WHERE SFID='" + textBox1 .Text + "'"))
            {

                if (bc.exists("SELECT * FROM SURFACE_PROCESSING WHERE SURFACE_PROCESSING='"+textBox2 .Text +"'"))
                {
                    hint.Text = "此表面加工已经存在";
                ...
                {
                    basec.getcoms(@"INSERT INTO SURFACE_PROCESSING(SFID,SURFACE_PROCESSING,MAKERID,DATE,YEAR,
            ...
            else
            {
                    ...
                    basec.getcoms(@"UPDATE SURFACE_PROCESSING SET SURFACE_PROCESSING='" + textBox2.Text + "',MAKERID='" + varMakerID +
                          "',DATE='" + varDate + "' WHERE SFID='" + textBox1.Text + "'");
                    IFExecution_SUCCESS = true;
                    bind();
        ...
        {
            ClearText();
            textBox1.Text = cSURFACE_PROCESSING.GETID();
            textBox2.Focus();
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dt = bc.getdt(sql+" WHERE A.SURFACE_PROCESSING LIKE '%"+textBox5 .Text +"%'");
                if (dt.Rows.Count > 0)
                {
                    dataGridView1.DataSource = dt;
                    dgvStateControl();
        ...
        private void btnDel_Click(object sender, EventArgs e)
        {
            ...
            {
                IFExecution_SUCCESS = false;
                string strSql = "DELETE FROM SURFACE_PROCESSING WHERE SURFACE_PROCESSING='" + id + "'";
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (v1 != "")
            {
                textBox1.Text = bc.getOnlyStringO("SURFACE_PROCESSING", "SFID", "SURFACE_PROCESSING", v1);
        ...
        private void dataGridView1_DoubleClick(object sender, EventArgs e)
        {
            if (select == 1)
            {
                BASE_INFO.PRINTING_TYPET.SURFACE_PROCESSING = dt.Rows[dataGridView1.CurrentCell.RowIndex]["表面加工"].ToString();
                BASE_INFO.PRINTING_TYPET.IF_DOUBLE_CLICK = true;
            }
            else
            {
                OFFER_MANAGE.PRINTING_OFFERT.SURFACE_PROCESSING = dt.Rows[dataGridView1.CurrentCell.RowIndex]["表面加工"].ToString();
                OFFER_MANAGE.PRINTING_OFFERT.IF_DOUBLE_CLICK = true;

            }
            this.Close();
        }
        public void PRINTING_TYPE_USE()
        {
