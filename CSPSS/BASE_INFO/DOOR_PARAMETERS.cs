...
{
    public partial class DOOR_PARAMETERS : Form
    {
        ...
        public DOOR_PARAMETERS()
        {
            InitializeComponent();
        }
        private void DOOR_PARAMETERS_Load(object sender, EventArgs e)
        {
        ...
        private void bind()
        {
            ...
            stb.Append(cDOOR_PARAMETERS.sql);
            stb.Append(" WHERE B.DOOR_PARAMETERS LIKE '%" + textBox2.Text + "%'");
            stb.Append(" AND A.PRICE LIKE '%" + textBox3.Text + "%'");
        ...
        {
            string sqlo = " ORDER BY B.DPID ASC";
 ...
 (SELECT USER_GROUP FROM USERINFO WHERE USID='" + LOGIN.USID + "'))" + sqlo);
        ...
        {
            //IDO = cDOOR_PARAMETERS.GETID();
            DOOR_PARAMETERST FRM = new DOOR_PARAMETERST(this);
        ...
        private void dataGridView1_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                if (GET_DATA_OR_UPDATE)
                {
                    ...
                    //OFFER_MANAGE.PRINTING_OFFERT.DOOR_PARAMETERS = dt.Rows[intCurrentRowNumber]["芯纸"].ToString();
...
FROM 
DOOR_PARAMETERS_MST 
WHERE DOOR_PARAMETERS='{0}'", dt.Rows[dataGridView1.CurrentCell.RowIndex]["印刷用纸或芯纸"].ToString()));
        ...
        private void btnToExcel_Click(object sender, EventArgs e)
        {
            if (dt.Rows.Count > 0)
            {
                ...
                bc.dgvtoExcel(dataGridView1, "材料门幅参数信息");
