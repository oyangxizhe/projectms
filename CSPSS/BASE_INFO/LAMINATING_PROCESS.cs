...
{
    public partial class LAMINATING_PROCESS : Form
    {
...
A.DATE AS 制单日期
FROM
LAMINATING_PROCESS A ";
   
        protected int M_int_judge, i;
        protected int select;
        public LAMINATING_PROCESS()
        {
            InitializeComponent();
        }
        ...
        private void dgvStateControl()
        {
            ...
            for (i = 0; i < numCols1; i++)
            {
                ...
                dataGridView1.Columns["裱纸工艺"].Width = 120;
        ...
        private void save()
        {
            ...
            if (!bc.exists("SELECT LPID FROM LAMINATING_PROCESS WHERE LPID='" +textBox1 .Text  + "'"))
            {

                if (bc.exists("SELECT * FROM LAMINATING_PROCESS WHERE LAMINATING_PROCESS='"+textBox2 .Text +"'"))
                {
                    hint.Text = "此裱纸工艺已经存在";
                ...
                {
                    basec.getcoms(@"INSERT INTO LAMINATING_PROCESS(LPID,LAMINATING_PROCESS,MAKERID,DATE,YEAR,
            ...
            else
            {
                    ...
                    basec.getcoms(@"UPDATE LAMINATING_PROCESS SET LAMINATING_PROCESS='" + textBox2.Text + "',MAKERID='" + varMakerID +
                          "',DATE='" + varDate + "' WHERE LPID='" +textBox1 .Text  + "'");
                    IFExecution_SUCCESS = true;
                    bind();
        ...
        {
            ClearText();
            textBox1 .Text  = cLAMINATING_PROCESS.GETID();
            textBox2.Focus();
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dt = bc.getdt(sql+" WHERE  A.LAMINATING_PROCESS LIKE '%"+textBox5 .Text +"%'");
                if (dt.Rows.Count > 0)
                {
                    dataGridView1.DataSource = dt;
                    dgvStateControl();
        ...
        private void btnDel_Click(object sender, EventArgs e)
        {
            ...
            {
                IFExecution_SUCCESS = false;
                string strSql = "DELETE FROM LAMINATING_PROCESS WHERE LAMINATING_PROCESS='" + id + "'";
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (v1 != "")
            {
                textBox1.Text = bc.getOnlyString("SELECT LPID FROM LAMINATING_PROCESS WHERE LAMINATING_PROCESS='"+v1+"'");
        ...
        private void dataGridView1_DoubleClick(object sender, EventArgs e)
        {
            if (select == 1)
            {
                BASE_INFO .MACHININGT.LAMINATING_PROCESS  = dt.Rows[dataGridView1.CurrentCell.RowIndex]["裱纸工艺"].ToString();
                BASE_INFO.MACHININGT .IF_DOUBLE_CLICK = true;
                this.Close();
            }
            else
            {
                OFFER_MANAGE.PRINTING_OFFERT.LAMINATING_PROCESS = dt.Rows[dataGridView1.CurrentCell.RowIndex]["裱纸工艺"].ToString();
                OFFER_MANAGE.PRINTING_OFFERT.IF_DOUBLE_CLICK = true;
                this.Close();
            }
        }
        public void MACHINING_USE()
        {
            select = 1;
