    ...
    public class basec
    {
        ...
        public string GET_SQLCONNECTION_STRING()
        {
            ...
            /* if (RETURN_SERVER_IP_OR_DOMAIN() == "192.168.1.9")
             ...
             {
                 url = "http://" + RETURN_SERVER_IP_OR_DOMAIN() + "/webserver/s_connectionstring.aspx";
             }
             ...
             else
            {
             ...
             //return M_str_sqlcon;*/
            return "Data Source=localhost;Database=OFFER;User id=sa;PWD=0";
        }
...
WHERE A.STEP_ID='{0}' ", CURRENT_STEP_ID);
        ...
        {
            string v = "";
            if (File.Exists(System.IO.Path.GetFullPath("项目管理系统.exe.config")))
            {
                //MessageBox.Show(GetSERVER_IP(System.IO.Path.GetFullPath("项目管理系统.exe.config")));
                v = RETURN_APPOINT_UNTIL_CHAR(GetSERVER_IP(System.IO.Path.GetFullPath("项目管理系统.exe.config")), 8, '/');
            }
            else
            {
                MessageBox.Show("不存在指定的配置文件" + System.IO.Path.GetFullPath("项目管理系统.exe.config"));
            }
            return v;
        }
        #endregion
        ...
        #region GetSERVER_IP
        public string GetSERVER_IP_1(string Dir)
        {
            //获取客户端应用程序及服务器端升级程序的最近一次更新版本
            string LastUpdateVersion = "";
            string AutoUpdaterFileName = Dir;
